...
paras = ["Health","Environmental Impact","Cost"];
////Set up database
...
function getForm(){
    ...
    //turn eentire thing into a string
    ...
    //add modified object (with encrypted original) to the database
...
function parameter(value,id){
    for(var i in paras){
        var a = document.getElementById(paras[i]).value;
        //ravel user
        var a = getObject();
...
function getObject(email){
    var request = db.transaction(["food"]).objectStore("food").objectStore.get("peterpan@gmail.com");
